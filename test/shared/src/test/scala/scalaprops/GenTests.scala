...
object GenTests extends Scalaprops {
  ...
  lazy val expectedBaseGen: Gen[Base] =
    MkGen.genericNonRecursiveCoproduct(
      Generic[Base],
      MkCoproductGen.ccons(
        MkGen.genericProduct(
          Generic[BaseDB],
  ...
  lazy val expectedT1TreeGen: Gen[T1.Tree] =
    MkGen.genericRecursiveCoproduct(
      T1.Tree.recursive,
      Generic[T1.Tree],
  ...
  lazy val expectedRecGen: Gen[Rec] =
    MkHListGen.hcons[FieldType[Witness.`'i`.T, Int], Record.`'s -> String`.T, Nat._1](
  ...
  lazy val expectedUnionGen: Gen[Un] =
    MkCoproductGen.ccons[FieldType[Witness.`'i`.T, Int], Union.`'s -> String`.T, Nat._1](
      genFieldType[Witness.`'i`.T, Int](Gen[Int]),
  ...
  val coproductNotBias = {
    ...
    Property.forAll { seed: Long =>
      val result = Gen[X].samples(
        listSize = 12000,
        seed = seed
      ).groupBy(_.getClass).map { case (k, v) => k -> v.size }
      result.foreach{ x =>
        val n = x._2
        if(n < 3000 || 5000 < n){
